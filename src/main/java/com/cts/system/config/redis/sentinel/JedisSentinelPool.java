...
public class JedisSentinelPool extends Pool<ShardedJedis> {
    ...
    public JedisSentinelPool(final GenericObjectPoolConfig poolConfig, List<String> masters, Set<String> sentinels) {
    ...
    public JedisSentinelPool(final GenericObjectPoolConfig poolConfig,List<String> masters, 
    		Set<String> sentinels, final String password) {
	    	...
	    	while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
	    		for (String sentinel : sentinels) {
					...
					try {
						...
						//多次尝试连接redis失败后，可以继续启动
					    log.warn("Cannot connect to sentinel running @ " + hap + ". Trying next one:"+e.getMessage());
					}
		    	}
